"""This code is modified based on Tensorflow implementation by Sophie Burkhardt. https://github.com/sophieburkhardt/dirichlet-vae-topic-models"""
...
class CRNTM(object):
    ...
    def __init__(self, 
        ...
        with tf.variable_scope('decoder'):
          if self.n_sample ==1:  # single sample
            ...
            n_causal_candidate = tf.shape(causal_candidate_idx)[0]
            n_anti_causal_candidate = tf.shape(anti_causal_candidate_idx)[0]
            self.cross_entropy_loss = tf.zeros((1))

            # Conditional logic using tf.cond
            def update_causal_loss():
              sampled_causal = tf.random.uniform(shape=[], minval=0, maxval=n_causal_candidate, dtype=tf.int32)
              return self.cross_entropy_loss + self.clr(self.label, self.x, causal_candidate_idx[sampled_causal][0], tf.constant([[1, 0]] * self.batch_size))

            def update_anti_causal_loss():
              sampled_causal = tf.random.uniform(shape=[], minval=0, maxval=n_anti_causal_candidate, dtype=tf.int32)
              return self.cross_entropy_loss + self.clr(self.label, self.x, anti_causal_candidate_idx[sampled_causal][1], tf.constant([[0, 1]] * self.batch_size))

            self.cross_entropy_loss = tf.cond(n_causal_candidate > 0, update_causal_loss, lambda: self.cross_entropy_loss)
            self.cross_entropy_loss = tf.cond(n_anti_causal_candidate > 0, update_anti_causal_loss, lambda: self.cross_entropy_loss)
        ...
        if self.n_sample ==1:
          ...
          with tf.variable_scope("decoder", reuse=True):
              ...
              self.kld2 = tf.abs(tf.contrib.distributions.Dirichlet(self.alpha).log_prob(self.doc_vec)-tf.contrib.distributions.Dirichlet(self.prior).log_prob(self.doc_vec))
              # ---------causal---------
              ...
              self.kld2_causal = tf.abs(tf.contrib.distributions.Dirichlet(self.alpha_causal).log_prob(self.doc_vec_causal)-tf.contrib.distributions.Dirichlet(self.prior_causal).log_prob(self.doc_vec_causal))
        ...
        logpi_gradient_causal = [tf.squeeze(separate_gradients(log_q(gamma_h(eps_causal, self.alpha_causal+tf.to_float(self.B),1.), self.alpha_causal+tf.to_float(self.B), 1.)+tf.log(dh(eps_causal, self.alpha_causal+tf.to_float(self.B), 1.)),var)) for var in enc_vars[5:10]]
    ...
    def get_dir_z(self,data, do_label):
        # encoder
        ...
        with tf.variable_scope('decoder', reuse=tf.AUTO_REUSE):
          if self.n_sample ==1:  # single sample
            ...
            #uniform variables for shape augmentation of gamma
            u_causal = tf.random_uniform((self.B,self.batch_size,self.dag_dim))
            with tf.variable_scope('prob_causal', reuse=tf.AUTO_REUSE):
                #this is the sampled gamma for this document, boosted to reduce the variance of the gradient
    ...
    def clr(self,label, data, sampled_u, expected_relation):
        do_causal_matrix = tf.eye(tf.shape(label)[1])
        do_label = tf.abs(label - do_causal_matrix[sampled_u])
        do_ouput_z = self.get_dir_z(data, do_label)[:, :self.n_topic]
        classify_ouputs = utils.create_counterfactual_contrastive_classifier(do_ouput_z)
        # classify_ouputs = utils.create_counterfactual_contrastive_classifier(tf.reshape(do_ouput_z, [tf.shape(label)[0], -1]))
        # cross_entropy_loss = tf.nn.softmax_cross_entropy_with_logits_v2(logits=classify_ouputs, labels=expected_relation)
...
def main(argv=None):
    ...
    crntm = CRNTM(analytical=analytical,
    ...
    train(sess, crntm, train_url, test_url,train_label_url,test_label_url,label_voc_path, FLAGS.batch_size,vocab_size,analytical,n_topic,y_dim,lexicon=lexicon,
